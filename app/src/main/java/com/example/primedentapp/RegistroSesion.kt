package com.example.primedentapp

import android.content.Intent
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.widget.Toast
import com.example.primedentapp.databinding.ActivityRegistroSesionBinding
import com.google.firebase.auth.FirebaseAuth

class RegistroSesion : AppCompatActivity() {

    private lateinit var binding:ActivityRegistroSesionBinding
    private lateinit var firebaseAuth: FirebaseAuth

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityRegistroSesionBinding.inflate(layoutInflater)
        setContentView(binding.root)

        firebaseAuth = FirebaseAuth.getInstance()

        binding.textView.setOnClickListener {
            val intent = Intent(this, InicioSesion::class.java)
            startActivity(intent)
        }

        binding.button.setOnClickListener {
            val email = binding.emailEt.text.toString()
            val pass = binding.passInputEditText.text.toString()
            val confirmPass = binding.passConfInputEditText.text.toString()


            if(email.isNotEmpty() && pass.isNotEmpty() && confirmPass.isNotEmpty()){
                if(pass ==confirmPass){
                    firebaseAuth.createUserWithEmailAndPassword(email,pass).addOnCompleteListener {
                        if(it.isSuccessful){
                            val intent = Intent(this, InicioSesion::class.java)
                            startActivity(intent)
                        }else{
                            Toast.makeText(this,it.exception.toString(), Toast.LENGTH_SHORT).show()
                        }
                    }

                }else{
                    Toast.makeText(this,"La constraseña no coincide", Toast.LENGTH_SHORT).show()
                }
            }else{
                Toast.makeText(this,"Ingrese datos porfavor", Toast.LENGTH_SHORT).show()
            }
        }
    }
}